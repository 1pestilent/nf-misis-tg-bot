...
async def download_current_week(message: Message, bot: Bot):
    ...
    first_day_week = timecalc.Monday()
    for course in dictionary.courses:
        ...
        if pngs:
            ...
            db = DataBase()
            await db.add_timetable(course = course, week = first_day_week.date())
            tt_id = await db.get_timetable_id()
            await db.add_pngs(ids = photo_id, timetable_id=tt_id)
        else:
            await bot.send_message(message.from_user.id, f'Расписание для {course}-ого курса на {first_day_week} уже загружено!')
            continue